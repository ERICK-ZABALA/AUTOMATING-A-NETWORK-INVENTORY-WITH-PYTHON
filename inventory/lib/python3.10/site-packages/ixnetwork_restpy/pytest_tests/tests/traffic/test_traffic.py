...
def test_can_add_raw_traffic_over_custom_stack(ixnetwork):
    ...
    vlan_protocol_template = ixnetwork.Traffic.ProtocolTemplate.find(
        StackTypeId="^vlan$"
    )
    ipv4_protocol_template = ixnetwork.Traffic.ProtocolTemplate.find(
        StackTypeId="^ipv4$"
    )
    ...
    udp_stack = config_element.Stack.read(
        ipv4_stack.AppendProtocol(udp_protocol_template)
    )
